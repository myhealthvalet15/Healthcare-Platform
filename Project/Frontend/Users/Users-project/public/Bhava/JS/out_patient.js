...
// Helper to create label/value rows
    ...
    apiRequest({
        ...
        onSuccess: (data) => {
            ...
            if (data.result && data.data && Array.isArray(data.data)) {
                ...
                // 🔍 Attach live search functionality after table is rendered
                setTimeout(() => {
                    ...
                    if (searchInput) {
                        searchInput.addEventListener('input', function () {
                            ...
                            rows.forEach(row => {
                                // If it's a loading or "no data" row, keep it visible
                ...
                }, 0); // Timeout ensures DOM is ready
            } else {
...
function createTableRow(entry, registryRelationships) {
    ...
    // 2. TYPE OF INCIDENT
   // 2. TYPE OF INCIDENT with colored dot
...
row.appendChild(createCell(diagnosis));
   ...
   // 9. DETAILS (icons/actions)
    ...
    if (badgeText === 'N') {
        const hasOpenChildren = registryRelationships?.has(entry.registry?.op_registry_id?.toString()) &&
            registryRelationships.get(entry.registry?.op_registry_id?.toString()) === true;
    ...
    viewLink.addEventListener('click', () => {
        fetch(`/UserEmployee/getHealthRegistryForEmployee/${entry.employee_id.toLowerCase()}/op/${entry.registry?.op_registry_id ?? ''}`, {
    ...
    body.innerHTML = `
        ...
        <p><strong>Symptoms/Injury:</strong> ${
            entry.registry?.type_of_incident === 'medicalIllness'
   ...
   function showTestListModal(dataset) {
    ...
    // ✅ Clear old content to prevent duplication
    testListElement.innerHTML = '';

    // Format and display test date
