...
namespace App\Http\Controllers\V1Controllers\HraController\Questions;
        ...
        try {
            ...
            if ($testIds) {
                foreach ($testIds as $testId) {
                    $testExists = MasterTest::where('master_test_id', $testId)->exists();
                    if (!$testExists) {
                        return response()->json(['error' => "Test ID {$testId} not found"], 400);
                    }
                }
            }
    ...
    public function getAllQuestions()
    {
        try {
            ...
            foreach ($questions as $question) {
                ...
                $template = HraTemplateQuestions::where('question_id', $question->question_id)->first();
                $question->template_id_linked_with = $template ? $template->template_id : null;
            }
            return response()->json([
                'data' => $questions,
            ], 200);
