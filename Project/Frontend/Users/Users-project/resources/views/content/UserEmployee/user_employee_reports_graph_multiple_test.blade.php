...
<style>
  ...
  #testResultTable table th,
  #testResultTable table td {
    padding: 4px 6px;
    text-align: center;
    white-space: nowrap;
    overflow: hidden;
  ...
  #testResultTable table tbody tr:nth-child(odd) {
    background-color: #fff;
  }

  #testResultTable table tbody tr:nth-child(even) {
    background-color: #f9f9f9;
  }

  #testResultTable table td:first-child {
    white-space: nowrap;
    text-align: center;
    width: 110px;
  }#testTablesContainer {
...
#testTablesContainer > div {
  flex: 1 1 48%;
  min-width: 250px;
}

#testTablesContainer table {
  width: 100%;
  border-collapse: collapse;
}

#testTablesContainer th,
#testTablesContainer td {
  padding: 4px 6px;
  text-align: center;
  white-space: nowrap;
  overflow: hidden;
...
<div class="row">
  ...
  <div id="testTablesContainer" style="margin-top: 20px; display: flex; gap: 20px; flex-wrap: wrap; justify-content: space-between;">
    ...
    apiRequest({
      ...
      onSuccess: function (response) {
        if (response.result && response.data && Array.isArray(response.data.subgroups)) {
          ...
          // Add static "Differential Count" option at the end
...
selectElement.appendChild(staticOptionLFT);
  ...
  document.getElementById('select2Primary_tests').addEventListener('change', function () {
    ...
    if (val === 'DIFF_COUNT_STATIC') {
      // Static differential count data
      const staticData = [
        { test_date: '2017-01-04', Neutrophils: 60, Lymphocyte: 30, Eosinophil: 3, Basophil: 1 },
        { test_date: '2019-06-05', Neutrophils: 65, Lymphocyte: 28, Eosinophil: 4, Basophil: 2 }
      ];
     updateChartWithData(staticData, 'differential');
     } else if (val === 'BP_STATIC') {
  // Static Blood Pressure data
  const staticBPData = [
    { test_date: '2017-01-04', systolic: 120, diastolic: 80 },
    { test_date: '2019-06-05', systolic: 120, diastolic: 80 },
    { test_date: '2023-01-05', systolic: 130, diastolic: 90 },
    { test_date: '2025-06-19', systolic: 135, diastolic: 95 }
  ];
  updateChartWithBPData(staticBPData);
} else if (val === 'LFT_BILIRUBIN_STATIC') {
  const lftData = [
    { test_date: '2017-01-04', total: 1.9, direct: 0.3, indirect: 1.6 },
    { test_date: '2019-06-05', total: 1.9, direct: 0.3, indirect: 1.6 },
    { test_date: '2019-12-02', total: 0.6, direct: 0.2, indirect: 0.4 },
    { test_date: '2023-08-20', total: 1.2, direct: 0.18, indirect: 1.02 },
    { test_date: '2025-05-10', total: 2.5, direct: 0.4, indirect: 2.1 }
  ];
  updateChartWithLFTData(lftData);
      ...
      // Fetch dynamic test data from API
      apiRequest({
  ...
  function updateChartWithDataForSingle(data) {
    ...
    new Chart(canvas.getContext('2d'), {
      ...
      options: {
        ...
        plugins: {
          legend: { position: 'top', align: 'start', labels: { usePointStyle: true, color: '#5e5873' } },
          tooltip: {
            callbacks: {
...
function updateChartWithLFTData(data) {
  ...
  // Add two new test entries
  const additionalData = [
    { test_date: '2023-03-15', total: 1.3, direct: 0.4, indirect: 0.9 },
    { test_date: '2024-11-10', total: 0.7, direct: 0.2, indirect: 0.5 }
  ];
  ...
  new Chart(canvas.getContext('2d'), {
    ...
    options: {
      ...
      scales: {
        ...
        y: { beginAtZero: false, title: { display: true, text: 'Bilirubin (mg/dL)' } }
      },
      plugins: {
        legend: { position: 'top', labels: { usePointStyle: true } },
        title: { display: true, text: 'Liver Function Test - Bilirubin Levels Over Time' }
      }
    }
  });

  // Generate test results table
  let tableHTML = `<table class="table table-bordered"><thead><tr><th>Date</th><th>Total</th><th>Direct</th><th>Indirect</th></tr></thead><tbody>`;
  data.forEach(d => {
    const dt = new Date(d.test_date);
    const formatted = `${String(dt.getDate()).padStart(2, '0')}-${String(dt.getMonth() + 1).padStart(2, '0')}-${dt.getFullYear()}`;
    tableHTML += `<tr><td>${formatted}</td><td>${d.total}</td><td>${d.direct}</td><td>${d.indirect}</td></tr>`;
  });
  tableHTML += `</tbody></table>`;

  // Reference range tables for Bilirubin + Lipid Profile separately
  const refTables = `
  ...
  testResultDiv.innerHTML = `<div style="width: 100%;">${tableHTML}</div><div style="display:flex;flex-wrap:wrap;margin-top:10px;">${refTables}</div>`;
  ...
  function updateChartWithDataForDifferential(data) {
    ...
    new Chart(canvas.getContext('2d'), {
      ...
      options: {
        ...
        plugins: {
          legend: { position: 'top', labels: { usePointStyle:true } },
          title: { display: true, text: 'Differential Count Over Time' }
        }
      }
    });
  ...
  // reference table (static example values — adjust if needed)
  const referenceHtml = `
  <table class="result-table" width="28%" style="float:left; margin: 0 2%;">
...
`;
  ...
  document.getElementById('testTablesContainer').style.display = 'flex';
    ...
    // legend
    const legendHTML = types.map((t,i) => `<div style="display:flex;align-items:center;">
  ...
  // Mock API request function - replace this with your real API helper
  function apiRequest({url, method, onSuccess, onError}) {
    fetch(url, { method })
      .then(res => res.json())
      .then(onSuccess)
      .catch(onError);
